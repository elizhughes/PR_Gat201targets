...
## Plot OD all stat_summary(geom -"line") remove excess lines RPMI for WT, GAT201 and dbl GAT201 P2.28EHonly

```{r plot_stat_summary_minus P2.28EH}

ggplot(data=normalisedOD  %>%
    dplyr::mutate(Strain = factor(Strain, levels = c("KN99 (WT)","GAT201","P2.28EH (dbl GAT201)")))%>%
                     filter(Strain != ""), 
                      aes(x=Time, y=OD_corrected, color=Strain)) + 
       ...
       labs(x = "Number of Days",
            y = "Absorbance (595nm)",
            title = "Double GAT201 Strain P2.28EH", subtitle = "RPMI at 37 degrees") +
...
ggsave("../Results/StatSummaryP2_28EH.png",width =7 ,height=5)
```
