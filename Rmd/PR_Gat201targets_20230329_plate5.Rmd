...
ggplot(data=normalisedOD  %>%
    dplyr::mutate(Strain = factor(Strain, levels = c("KN99 (WT)","GAT201","CAS3","CCP1","ISP6", "P2.7ZH (dbl GAT201)")))%>%
                     filter(Strain != ""), 
...
ggsave("../Results/AllSummary_PR5.png",width = 6.6,height=5)
```
...
ggplot(data=normalisedOD  %>%
    dplyr::mutate(Strain = factor(Strain, levels = c("KN99 (WT)","GAT201","CAS3","CCP1","ISP6")))%>%
                     filter(Strain != ""), 
                      aes(x=Time, y=OD_corrected, color=Strain)) + 
       #geom_line(aes(group = Well), size = 0.2, alpha = 0.2) + 
       stat_summary(fun = "median", geom = "line", size = 1) +
...
ggsave("../Results/AllSummary_PR5nodblGAT201.png",width = 6.6,height=5)
```
