---
title: "PRGAT201Targets_2023032_plate4"
...
rawdata <- read.csv("../Input/20230321_RPMI_Gat201Targets_Plate4_csv.csv")
...
platemap <- read.csv("../Input/20230321_RPMI_Gat201Targets_Plate4_setup.csv") %>%      dplyr::mutate(
                    Strain = factor(Strain, 
                    levels = c("KN99 (WT)", "GAT201", "BLP2", "CFO1","BLP4"),
                    labels = c("WT", "gat201∆", "blp2∆", "cfo1∆","blp4∆"))
  )
head(platemap, n=10)
...
Remove well A2-7, H2-10, B1,D1,F1
...
ggplot(data=normalisedOD %>%
                      filter(Strain == "cfo1∆"), 
                      aes(x=Time, y=OD_corrected, ccolour=Biorep, group = Well)) + 
       geom_line() + 
...
```{r plot_stat_summary_4_all}

ggplot(data=normalisedOD  %>%
    #dplyr::mutate(Strain = factor(Strain, levels = c("KN99 (WT)","GAT201","BLP2","BLP4","CFO1")))%>%
                     filter( !is.na(Strain) ),  
...
```{r save_RMPI_PR4_all.png}

ggsave("../Results/AllSummaryPR4.png",width = 6.6,height=5)
```

## Plot OD all stat_summary(geom -"line") remove excess lines RPMI CFO1 only

```{r plot_stat_summary_4_cfo1}

ggplot(data=normalisedOD  %>%
   
                     filter(Strain == "cfo1∆"),  
                      aes(x=Time, y=OD_corrected)) + 
       geom_line(aes(group = Well), size = 0.2, alpha = 0.5, colour = "purple") + 
       stat_summary(fun = "median", geom = "line", size = 1) +
       scale_y_continuous(limits=c(0,0.17,expand=c(0,0), breaks = c(0, 0.05, 0.15))) +
   
  
       labs(x = "Number of Days",
            y = "Absorbance (595nm)",
            title = "cfo1∆", subtitle = "Plate 4: RPMI at 37 degrees") +
       theme_bw() +
       theme(text = element_text(size = 15))+
       theme(axis.title = element_text(size = 15))+
       theme(axis.ticks.length = unit(0.3, "cm"))+
...
```{r save_RMPI_PR4_cfo1.png}

ggsave("../Results/AllSummaryPR4_cfo1.png",width = 5,height=5)
```


## Plot OD all stat_summary(geom -"line") Highlight summary line RPMI

```{r plot_all_stat_summary_4}
...
```{r save_RMPI_PR4_summ.png}

ggsave("../Results/StatSummaryPlate4.png",width = 6.6,height=5)
```
